...
export default function ProfileClient({ user, streak }: Props) {
  ...
  return (
    <div className="py-4 space-y-6 lg:py-0 lg:space-y-8">
      <h1 className="font-headline text-3xl lg:text-4xl font-extrabold text-on-surface">Profile</h1>

      {/* Avatar & Name + Streak Summary */}
      <div className="lg:grid lg:grid-cols-5 lg:gap-6 space-y-6 lg:space-y-0">
        <div className="bg-surface-container-lowest rounded-3xl p-6 lg:p-8 flex items-center gap-4 lg:col-span-3">
          <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-gradient-to-br from-primary to-primary-container flex items-center justify-center text-on-primary font-headline text-2xl lg:text-3xl font-extrabold shrink-0">
            {initials}
          </div>
          <div className="min-w-0">
            <h2 className="font-headline text-xl lg:text-2xl font-bold text-on-surface truncate">{user.name}</h2>
        ...
        <div className="grid grid-cols-3 gap-3 lg:col-span-2 lg:grid-cols-1 lg:gap-3">
          {[
            ...
            <div key={s.l} className="bg-surface-container-lowest rounded-2xl p-3 lg:p-4 text-center lg:flex lg:items-center lg:gap-4 lg:text-left">
              <span className="material-symbols-outlined text-primary text-xl lg:text-2xl block mb-1 lg:mb-0 lg:shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>{s.icon}</span>
              <div className="lg:flex-1">
                <div className="font-headline font-extrabold text-on-surface lg:text-xl">{s.v}</div>
                <div className="text-[10px] lg:text-xs text-on-surface-variant font-semibold">{s.l}</div>
              </div>
            </div>
          ))}
        </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {[5, 10, 15, 20].map(n => (
                  <button key={n} type="button" onClick={() => setForm({ ...form, dailyGoal: n })}
                    className={`py-2 lg:py-3 rounded-xl font-bold text-sm border-2 transition-all ${form.dailyGoal === n ? "border-primary bg-primary-fixed/20 text-primary" : "border-transparent bg-surface-container-high text-on-surface-variant hover:bg-surface-container"}`}>
                    {n}
                  </button>
                ))}
              </div>
      ...
      <div className="bg-surface-container-lowest rounded-3xl p-5 lg:p-7">
        ...
        {securityOpen && (
          <div className="mt-4 space-y-3 lg:grid lg:grid-cols-3 lg:gap-4 lg:space-y-0">
            ...
            <div className="flex gap-2 pt-1 lg:col-span-3">
              <button
                onClick={() => { setSecurityOpen(false); setPwForm({ currentPassword: "", newPassword: "", confirmPassword: "" }); }}
                className="flex-1 py-3 rounded-xl border-2 border-outline-variant text-on-surface-variant font-semibold text-sm hover:bg-surface-container transition-colors lg:flex-none lg:px-8"
              >
                Cancel
              </button>
              <button
                ...
                disabled={changingPassword}
                className="flex-1 py-3 rounded-xl bg-primary text-on-primary font-bold text-sm disabled:opacity-60 flex items-center justify-center gap-1 hover:opacity-90 transition-opacity lg:flex-none lg:px-8"
              >
                {changingPassword ? (
                  <>
      ...
      <button
        ...
        className="w-full py-4 rounded-2xl border-2 border-error/30 text-error font-bold font-headline flex items-center justify-center gap-2 hover:bg-error-container/20 transition-colors lg:max-w-md lg:mx-auto"
      >
